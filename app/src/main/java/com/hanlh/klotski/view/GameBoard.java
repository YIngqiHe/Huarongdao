package com.hanlh.klotski.view;
...
import com.hanlh.klotski.entry.Record;
import com.hanlh.klotski.entry.Block;
...
public class GameBoard extends RelativeLayout {
    ...
    public void setSuccess(boolean success) {
        isSuccess = success;
        // 记录关卡
        String strLevel = SpUtils.get(SpUtils.LEVEL);
        //过第一个关卡
        if (TextUtils.isEmpty(strLevel)) {
            SpUtils.put(SpUtils.LEVEL, 1 + "");
        }
        //当前关卡是
        else if (index == Integer.valueOf(strLevel)) {
            SpUtils.put(SpUtils.LEVEL, (index + 1) + "");
        }
        index += 1;
    }
...
//        init(attrs, defStyle);
    ...
    public void initBorder() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            border[i].setBackgroundColor(getResources().getColor(R.color.colorBlockBackground));
        }
